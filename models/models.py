...
##把药物组合 + 蛋白质信息 → 转换为统一 hidden size 的 embedding 序列，作为 Transformer 输入。
...
##把这些向量送进 BERT/Transformer 编码器里，最后输出药物组合的协同预测结果。
...
## 用异构图神经网络去学习 drug / protein / sideeffect 等不同类型节点的“全局关系表示”，并且还能顺带做链路预测训练。
...
## 把 micro feature 和 macro feature 融合，然后预测药物协同概率。
